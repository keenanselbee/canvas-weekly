...
| As much useful course information as safely available | Metadata, own submission status, stored syllabus and course-tagged messages; retained stale evidence and explicit source coverage | Assignment instructions, Canvas page/file contents and modules remain excluded due to unproven or side-effectful access paths; full course completeness is not achieved |
...
The old development app process was still running when reviewed. It predates the
latest changes. A request to the user to close it remains unanswered. The Windows
Computer Use helper was unavailable, so no replacement UI control was attempted.
