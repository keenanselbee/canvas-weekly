...
export function validatePriorities(result, evidence) {
  ...
  for (const item of result.priorities) {
    ...
    for (const step of item.steps) {
      ...
      if ((source.authorUnverified || source.authorRoleUnverified) && step.kind !== 'suggested') throw new Error('ChatGPT treated an unverified message as a course requirement.');
