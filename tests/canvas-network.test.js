...
test('network admission requires exact reviewed paths and parameters', () => {
  ...
  for (const operation of ['profile', 'courses', 'course', 'assignments', 'quizzes', 'pages', 'files', 'groups', 'announcements', 'conversations', 'conversation', 'calendar']) {
    assert.equal(permittedRead(requestUrl(origin, operation, { courseId: 1, conversationId: 7 }), origin), true, operation);
  }
  for (const route of ['/api/v1/conversations/7', '/api/v1/conversations/7?auto_mark_as_read=true', '/api/v1/courses/1/modules', '/api/v1/courses/1/assignments?include[]=submission', '/api/v1/users/self/profile?delete=true', '/api/v1/users/self/profile?per_page=1000']) {
