...
function fixture({ account = { accountMembership: 'none', studentId: binding.studentId, globalUserId: binding.globalUserId }, rows = [enrollment('10'), enrollment('11')] } = {}) {
  ...
  const transport = {
    ...
    async request(value) {
      ...
      const assignments = operationName === 'CanvasWeeklyAssignments';
      const nodes = assignments ? [{ _id: '20', courseId: '1', name: 'Preparation', state: 'published', pointsPossible: 5, submissionTypes: ['online_upload'] }]
        : [{ _id: '30', assignmentId: '20', state: 'unsubmitted', cachedDueDate: '2026-09-18T18:00:00Z' }];
      return { data: { course: { _id: '1', name: 'Example course', courseCode: 'EX 1',
        [assignments ? 'assignmentsConnection' : 'submissionsConnection']: { nodes, pageInfo: { hasNextPage: false, endCursor: null } } } } };
    },
  };
...
test('student collection completes both preflights before metadata and exports no role evidence', async () => {
  ...
  const order = ['account', 'CanvasWeeklyEnrollmentScope', 'CanvasWeeklyEnrollmentScopesecond', 'CanvasWeeklyAssignments', 'CanvasWeeklySubmissionStates'];
...
test('cancellation after preflight or metadata discards late results and prevents subsequent stages', async () => {
  for (const stopAt of ['account', 'CanvasWeeklyEnrollmentScopesecond', 'CanvasWeeklySubmissionStates']) {
    const controller = new AbortController();
    const { calls, transport } = fixture();
    for (const method of ['checkAccountMembership', 'request']) {
